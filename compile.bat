...
for /f "delims=" %%a in ( 'dir ext\SDL2-* /b' ) do set sdl=%%a
for /f "delims=" %%a in ( 'dir ext\SDL2_ttf* /b' ) do set sdl_ttf=%%a
for /f "delims=" %%a in ( 'cd' ) do set pwd=%%a
set outdir=bin
cl /EHsc /MD  ^
src\main.cpp /Fe%outdir%\test.exe ^
/I %pwd%\ext\%sdl%\include ^
/I %pwd%\ext\%sdl_ttf%\include ^
/link ^
/LIBPATH:%pwd%\ext\%sdl%\lib\x64 ^
/LIBPATH:%pwd%\ext\%sdl_ttf%\lib\x64 ^
shell32.lib SDL2.lib SDL2_ttf.lib SDL2main.lib ^
/SUBSYSTEM:CONSOLE

::cp -n "%pwd%\ext\%sdl%\lib\x64\SDL2.dll" %outdir%\
echo n |copy /-y "%pwd%\ext\%sdl%\lib\x64\*.dll" %outdir%\
echo n |copy /-y "%pwd%\ext\%sdl_ttf%\lib\x64\*.dll" %outdir%\
